...
| | Where | Runs | Purpose |
|---|---|---|---|
| **dev** | this Mac | `gridline/dev/` compose, or the hot-reload devenv | Build and try changes |
| **prod** | home lab | `gridline/prod/` compose, GHCR images | The instance the team actually designs in |

Everything we add lives under `gridline/` and `.github/workflows/gridline-*.yml`
...
- `gridline` — our line. Pushes here build `:dev` images.
...
```bash
gridline/scripts/bootstrap-mac.sh            # check what's missing
gridline/scripts/bootstrap-mac.sh --install  # install colima + docker, start the VM
```
...
docker compose -p gridline-dev --env-file .env -f docker-compose.yaml up -d
...
`ghcr.io/amannigam/gridline-*:dev` images instead.
...
- push to `gridline` → `ghcr.io/amannigam/gridline-{frontend,backend,exporter,mcp}:dev`
...
```bash
gridline/scripts/deploy-homelab.sh            # uses the `homelab` ssh host
```

First run seeds `/home/claw/gridline/.env` from the example and stops so you can fill
...
```bash
git tag -a v0.1.0 -m "gridline 0.1.0" && git push origin v0.1.0
# wait for CI, then:
ssh homelab "sed -i 's/^PENPOT_VERSION=.*/PENPOT_VERSION=v0.1.0/' /home/claw/gridline/.env"
gridline/scripts/deploy-homelab.sh
```
...
```bash
gridline/scripts/backup.sh
```

Penpot's entire state is the Postgres database plus the assets volume. The script
pulls both to `~/backups/gridline` and prints the restore commands. Run it before
...
```bash
gridline/scripts/sync-upstream.sh          # what's new
gridline/scripts/sync-upstream.sh --merge  # merge upstream/main into gridline
```
