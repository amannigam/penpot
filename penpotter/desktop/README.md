...
```bash
cd penpotter/desktop
npm install
npm start              # home lab instance
npm run start:dev      # local dev stack on :9101
PENPOTTER_URL=http://localhost:3449 npm start   # anything else, e.g. the devenv
```

## Build a DMG

```bash
npm run dist           # arm64 -> dist/Penpotter-0.1.0-arm64.dmg
...
/Applications/Penpotter.app`. Signing + notarisation is a paid Apple account;
