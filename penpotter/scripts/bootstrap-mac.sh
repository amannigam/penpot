#!/usr/bin/env bash
# Prepare this Mac to run and build Penpotter.
#
#   penpotter/scripts/bootstrap-mac.sh          # check only
#   penpotter/scripts/bootstrap-mac.sh --install # install what's missing
#
...
COLIMA_PROFILE="${COLIMA_PROFILE:-penpotter}"
...
info "Docker is ready. Next: penpotter/dev/ — cp .env.example .env, then"
echo "  docker compose -p penpotter-dev --env-file penpotter/dev/.env -f penpotter/dev/docker-compose.yaml up -d"
