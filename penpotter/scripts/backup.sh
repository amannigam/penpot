#!/usr/bin/env bash
# Back up the production Penpotter instance: Postgres dump + assets tarball.
#
#   penpotter/scripts/backup.sh [ssh-host] [local-dest-dir]
#
...
DEST="${2:-${PENPOTTER_BACKUP_DIR:-$HOME/backups/penpotter}}"
...
ssh "$HOST" "docker run --rm -v ${PROJECT}_penpotter_assets:/assets:ro alpine tar -C /assets -cz ." \
...
Restore (onto a stack running the SAME image version):
  ...
  cat penpot-assets-$STAMP.tar.gz | ssh $HOST 'docker run --rm -i -v ${PROJECT}_penpotter_assets:/assets alpine tar -C /assets -xz'
EOF
