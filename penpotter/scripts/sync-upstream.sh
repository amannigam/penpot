...
#
#   penpotter/scripts/sync-upstream.sh          # fetch + report what's new
#   penpotter/scripts/sync-upstream.sh --merge  # also merge upstream/main into penpotter
#
# We track upstream `main` (the stable release line), not `develop`. Our own
# work lives on the `penpotter` branch; everything we add is confined to
# penpotter/ and .github/workflows/penpotter-*.yml so merges stay boring.
set -euo pipefail
cd "$(dirname "$0")/../.."
